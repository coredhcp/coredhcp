...
import (
	...
	"github.com/coredhcp/coredhcp/conn"
...
// Opt adds optional configuration to a server.
type Opt func(s *Server)

// WithConn configures the server with the given connection.
func WithConn(c net.PacketConn) Opt {
...
func NewServer(addr *net.UDPAddr, handler Handler, iface string, opt ...Opt) (*Server, error) {
	s := &Server{
		Handler: handler,
	}
	...
	if s.conn == nil {
		var err error
		conn, err := conn.NewUDP4BoundListener(iface, addr.String())
		if err != nil {
			return nil, err
		}
		s.conn = conn
